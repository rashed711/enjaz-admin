...
const SuppliersListPage: React.FC = () => {
    ...
    const suppliers = useMemo(() => {
        return accountsFlat.filter(acc => acc.party_type === PartyType.SUPPLIER || acc.party_type === PartyType.CUSTOMER_AND_SUPPLIER);
    }, [accountsFlat]);
    ...
    const filteredSuppliers = useMemo(() => {
        ...
        return suppliers.filter(supplier =>
            ...
            supplier.code.toLowerCase().includes(debouncedSearchQuery.toLowerCase())
        );
    }, [suppliers, debouncedSearchQuery]);

    const handleSupplierClick = (supplierId: number) => {
        navigate(`/accounts/suppliers/${supplierId}/statement`);
    };

    return (
        <>
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
                <h2 className="text-2xl font-bold text-text-primary">قائمة الموردين</h2>
            ...
            {accountsLoading ? (
                ...
                <EmptyState
                    Icon={UsersIcon}
                    title={searchQuery ? 'لا توجد نتائج' : 'لا يوجد موردين'}
                    message={searchQuery ? 'لم نجد أي موردين يطابقون بحثك.' : 'لم يتم إضافة أي حسابات موردين بعد. يمكنك إضافتهم من دليل الحسابات.'}
                />
            ) : (
                <div className="bg-card rounded-lg shadow-sm border border-border overflow-x-auto">
                    <table className="w-full text-right min-w-[600px] text-sm">
                        <thead className="bg-slate-50">
                            <tr>
                                <th className="px-3 py-3 font-bold text-text-secondary">اسم المورد</th>
                        ...
                        <tbody className="text-text-primary divide-y divide-border">
                            {filteredSuppliers.map((supplier) => (
                                <tr key={supplier.id} className="hover:bg-slate-100 even:bg-slate-50/50 cursor-pointer" onClick={() => handleSupplierClick(supplier.id)}>
                                    <td className="px-3 py-2 font-semibold">{supplier.name}</td>
                                    <td className="px-3 py-2 font-mono">{supplier.code}</td>
