...
const formatQuotation = (q: any): Quotation => ({
    ...
    // Fields from the new view
    convertedInvoiceId: q.converted_invoice_id,
    convertedInvoiceNumber: q.converted_invoice_number,
...
const QuotationsListPage: React.FC = () => {
    ...
    const { items: quotations, loading, totalCount, currentPage, setCurrentPage, itemsPerPage } = usePaginatedList({
        ...
        tableName: 'quotations_with_status',
                    ...
                    <div className="hidden lg:block bg-card rounded-lg shadow-sm border border-border overflow-x-auto">
                        <table className="w-full text-right min-w-[800px] text-sm">
                            ...
                            <tbody className="text-text-primary divide-y divide-border">
                                {quotations.map((q) => {
                                    const isConverted = !!q.convertedInvoiceId;
                                    return (
                                        <tr
                                            key={q.id}
                                            className={`hover:bg-slate-100 even:bg-slate-50/50 cursor-pointer ${isConverted ? 'text-slate-400' : ''}`}
                                        ...
                                        >
                                            ...
                                            <td className="px-3 py-3 whitespace-nowrap">
                                                {isConverted ? (
                                                    <a 
                                                        href={`/sales-invoices/${q.convertedInvoiceId}/view`}
                                                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); navigate(`/sales-invoices/${q.convertedInvoiceId}/view`); }}
                    ...
                    <div className="lg:hidden space-y-4">
                        {quotations.map((q) => {
                            ...
                            return (
                                <div
                                    key={q.id}
                                    className={`bg-card border border-border rounded-lg p-4 shadow-sm active:bg-slate-50 even:bg-slate-50/50 ${isConverted ? 'text-slate-400' : ''}`}
                                    onClick={() => navigate(`/quotations/${q.id}/view`)}
                                >
                                    <div className="flex justify-between items-start mb-3">
                                        <p className={`font-bold text-lg ${isConverted ? 'text-slate-400' : 'text-primary'}`}>{q.quotationNumber}</p>
