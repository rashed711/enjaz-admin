...
const CustomerStatementPage: React.FC = () => {
    ...
    const customer = useMemo(() => {
        if (accountsLoading || !customerId) return null;
        return accountsFlat.find(acc => acc.id === customerId);
    }, [accountsFlat, accountsLoading, customerId]);
    ...
    useEffect(() => {
        ...
        const fetchStatement = async () => {
            ...
            try {
                ...
                console.error("Failed to fetch customer statement:", e);
    ...
    const handleShare = async () => {
        ...
        const fileName = `كشف حساب - ${customer.name}.pdf`;
        const file = new File([blob], fileName, { type: 'application/pdf' });
        const shareData = {
            files: [file],
            title: `كشف حساب العميل ${customer.name}`,
            text: `مرفق كشف حساب للعميل ${customer.name}.`,
        };
    ...
    if (!customer) {
        return <div className="p-4 text-center text-red-500">لم يتم العثور على العميل.</div>;
    }
    ...
    return (
        <>
            ...
            {statementWithBalance.length === 0 ? (
                <EmptyState Icon={DocumentTextIcon} title="لا توجد حركات" message="لا توجد أي قيود محاسبية مسجلة لهذا العميل حتى الآن." />
            ) : (
                <StatementComponent 
                    party={customer}
                    partyTypeLabel="العميل"
