        ...
        if (empty($errors)) {
            $db->prepare("
                INSERT INTO client_subscriptions
                  (client_id, service_id, plan_name, currency, original_price, price, start_date, end_date, notes, status, created_by)
                VALUES (?,?,?,?,?,?,?,?,?,'active',?)
            ")->execute([$clientId, $serviceId, $planName, $currency, $originalPrice, $price, $dbValStartDate, $dbValEndDate, $notes, currentUserId()]);
